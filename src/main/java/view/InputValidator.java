...
public class InputValidator {
    ...
    private static final int NAME_LENGTH_MIN_BOUND = 1;
    private static final int NAME_LENGTH_MAX_BOUND = 5;
    ...
    private static final int COUNT_MIN_BOUND = 1;
    private static final int COUNT_MAX_BOUND = 100;
    ...
    private void validateNameLength(final String name) {
        if (isInvalidNameLength(name)) {
            throw new IllegalArgumentException(
                    format(INVALID_NAME_MESSAGE, NAME_LENGTH_MIN_BOUND, NAME_LENGTH_MAX_BOUND)
            );
        }
    }

    private boolean isInvalidNameLength(final String name) {
        return name.length() < NAME_LENGTH_MIN_BOUND || NAME_LENGTH_MAX_BOUND < name.length();
    }
    ...
    public void validateCount(final int count) {
        if (isInvalidCount(count)) {
            throw new IllegalArgumentException(
                    format(INVALID_COUNT_MESSAGE, COUNT_MIN_BOUND, COUNT_MAX_BOUND)
            );
        }
    }

    private boolean isInvalidCount(final int count) {
        return count < COUNT_MIN_BOUND || COUNT_MAX_BOUND < count;
    }
}